...
def main():
    ...
    # small subset for a quick run; increase later
    ...
    # DistilBERT uses q_lin/v_lin in attention blocks
    lora = LoraConfig(r=8, lora_alpha=16, target_modules=["q_lin","v_lin"], lora_dropout=0.05, bias="none")
    ...
    tr = Trainer(model=model, args=args, train_dataset=enc_ds["train"],
                 eval_dataset=enc_ds["test"], compute_metrics=metrics)
    tr.train()
    print(tr.evaluate())
